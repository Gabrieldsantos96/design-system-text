...
import { B3PaginationComponent } from './pagination.component';

@Component({
  selector: 'test-host-component',
  standalone: true,
  imports: [B3PaginationComponent],
  template: ` <b3-pagination [zPageIndex]="pageIndex" [zTotal]="totalPages" [zSize]="'default'" (zPageIndexChange)="onPageChange($event)" [class]="customClass"></b3-pagination> `,
})
...
describe('B3PaginationComponent Integration (Jest)', () => {
  let fixture: ComponentFixture<TestHostComponent>;
  let hostComponent: TestHostComponent;
  let nativeEl: HTMLElement;

  const getButtons = (): NodeListOf<HTMLButtonElement> => nativeEl.querySelectorAll('b3-pagination-button > button');
  ...
  it('should render the correct number of page buttons (including prev and next)', () => {
    const pageButtons = nativeEl.querySelectorAll('b3-pagination-button');
  ...
  it('should apply custom class to pagination root element', () => {
    const paginationHost = nativeEl.querySelector('b3-pagination');
