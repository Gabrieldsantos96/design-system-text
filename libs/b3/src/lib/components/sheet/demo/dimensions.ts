...
@Component({
  selector: 'b3-demo-sheet-dimensions',
  standalone: true,
  imports: [B3ButtonComponent],
  template: `
    <div class="flex flex-wrap gap-4">
      <button b3-button zType="outline" (click)="openWideSheet()">Wide Sheet (500px)</button>
      <button b3-button zType="outline" (click)="openTallSheet()">Tall Sheet (80vh)</button>
      <button b3-button zType="outline" (click)="openCustomSheet()">Custom Dimensions</button>
      <button b3-button zType="outline" (click)="openTopSheet()">Top Sheet</button>
    </div>
  `,
})
export class B3DemoSheetDimensionsComponent {
