...
@Component({
  selector: 'b3-demo-date-picker-sizes',
  standalone: true,
  ...
  template: `
    <div class="flex flex-col gap-4">
      <div class="space-y-2">
        ...
        <b3-date-picker zSize="sm" [value]="selectedDateSm()" placeholder="Pick a date" (dateChange)="onDateChangeSm($event)" />
      </div>

      <div class="space-y-2">
        <h4 class="text-sm font-medium">Default</h4>
        <b3-date-picker zSize="default" [value]="selectedDateDefault()" placeholder="Pick a date" (dateChange)="onDateChangeDefault($event)" />
      </div>

      <div class="space-y-2">
        <h4 class="text-sm font-medium">Large</h4>
        <b3-date-picker zSize="lg" [value]="selectedDateLg()" placeholder="Pick a date" (dateChange)="onDateChangeLg($event)" />
      </div>
    </div>
  `,
})
export class B3DemoDatePickerSizesComponent {
