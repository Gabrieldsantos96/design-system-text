...
@Component({
  template: `
    <button b3-menu [zMenuTriggerFor]="menuPanel" [zDisabled]="disabled" [zTrigger]="trigger" [zHoverDelay]="hoverDelay">Trigger</button>
    <div #menuPanel></div>
  `,
  imports: [B3MenuDirective],
})
class TestComponent {
  disabled = false;
...
describe('B3MenuDirective', () => {
  let component: TestComponent;
  let fixture: ComponentFixture<TestComponent>;
  let directive: B3MenuDirective;
  ...
  beforeEach(async () => {
    ...
    const directiveDebugElement = fixture.debugElement.query(By.directive(B3MenuDirective));
    directive = directiveDebugElement.injector.get(B3MenuDirective);
  ...
  it('should determine if menu should stay open based on related target', () => {
    ...
    mockMenuElement.setAttribute('b3-menu-content', '');
