...
export class B3StringTemplateOutletDirective<_T = unknown> implements OnChanges {
  private readonly viewContainer = inject(ViewContainerRef);
  private readonly templateRef = inject(TemplateRef<unknown>);

  private embeddedViewRef: EmbeddedViewRef<unknown> | null = null;
  private readonly context = new B3StringTemplateOutletContext();
  @Input() zStringTemplateOutletContext: any | null = null;
  @Input() zStringTemplateOutlet: unknown | TemplateRef<unknown> = null;

  static ngTemplateContextGuard<T>(_dir: B3StringTemplateOutletDirective<T>, _ctx: unknown): _ctx is B3StringTemplateOutletContext {
    return true;
  }
...
export class B3StringTemplateOutletContext {
