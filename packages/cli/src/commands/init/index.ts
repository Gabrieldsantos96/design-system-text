...
export const init = new Command()
  ...
  .action(async options => {
    ...
    logger.info(isReInitializing ? 'Re-initializing b3-design-system...' : 'Initializing b3-design-system...');
    logger.break();
...
async function confirmReinitialization(): Promise<boolean> {
  logger.warn('b3-design-system is already initialized in this project.');
...
function displaySuccessMessage(config: any): void {
  logger.break();
  logger.success('b3-design-system has been initialized successfully!');
  logger.break();
  ...
  logger.info(chalk.bold(`  ${runCommand} @ngb3/ui add [component]`));
  logger.break();
}
