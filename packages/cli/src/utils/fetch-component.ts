...
const GITHUB_API = 'https://api.github.com/repos/b3-ui/b3-design-system/contents';
...
const GITHUB_RAW = `https://raw.githubusercontent.com/b3-ui/b3-design-system/${GITHUB_REF}`;

async function fetchFromGitHubAPI(filePath: string): Promise<string> {
  try {
    const apiUrl = `${GITHUB_API}/${filePath}?ref=${GITHUB_REF}`;
    const { stdout } = await execa('curl', ['-s', '-H', 'Accept: application/vnd.github.v3+json', '-H', 'User-Agent: b3-cli', apiUrl]);
    ...
    const { stdout } = await execa('curl', ['-s', '-H', 'User-Agent: b3-cli', rawUrl]);
...
export async function fetchComponentFromGithub(componentName: string, fileName: string, config: Config): Promise<string> {
  try {
    const filePath = `libs/b3/src/lib/components/${componentName}/${fileName}`;
