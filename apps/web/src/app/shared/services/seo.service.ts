...
export class SeoService {
  ...
  setMetaTags(config: SeoConfig): void {
    ...
    this.meta.updateTag({ property: 'og:site_name', content: 'B3 UI' });
  ...
  setHomeSeo(): void {
    this.setMetaTags({
      title: 'B3 UI - The @shadcn/ui Alternative for Angular',
  ...
  private generateDocsKeywords(title: string): string[] {
    return [`B3 UI ${title}`, `zard-ui ${title.toLowerCase()}`, `Angular B3/ui ${title}`, `B3 UI docs ${title}`, `shadcn/ui angular ${title}`, `zard ${title}`];
  }
