...
@Component({
  selector: 'b3-simple-code-highlight',
  standalone: true,
  template: `
    <div class="relative">
      @if (showCopyButton()) {
        <button (click)="copyCode()" class="absolute top-2 right-2 b3-10 p-2 rounded-md bg-muted hover:bg-muted/80 text-muted-foreground transition-colors" title="Copy code">
