...
import { B3BreadcrumbModule } from '@b3/components/sheet/sheet.module';

import type { FileTreeNode, BlockFile } from '../block-container/block-container.component';

@Component({
  selector: 'b3-file-tree',
  standalone: true,
  imports: [CommonModule, FileTreeComponent, B3BreadcrumbModule, B3IconComponent],
  template: `
    @for (node of nodes(); track node.path) {
      @if (node.type === 'folder' && node.path) {
        <div class="folder-item">
          <button (click)="toggleFolder(node.path)" b3-button zType="ghost" class="w-full justify-start ring-sidebar-ring">
            <b3-icon zType="chevron-right" zSize="sm" [class.rotate-90]="isOpen(node.path)" />
            <b3-icon zType="folder" zSize="sm" />
        ...
        >
          <b3-icon zType="file" zSize="sm" class="flex-shrink-0" />
