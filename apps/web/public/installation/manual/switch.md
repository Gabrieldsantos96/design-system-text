...
import { switchVariants, type B3SwitchVariants } from './switch.variants';
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => B3SwitchComponent),
      multi: true,
    },
  ],
...
export class B3SwitchComponent implements ControlValueAccessor {
  readonly checkChange = output<boolean>();
  readonly class = input<ClassValue>('');

  readonly zType = input<B3SwitchVariants['zType']>('default');
  readonly zSize = input<B3SwitchVariants['zSize']>('default');
...
export type B3SwitchVariants = VariantProps<typeof switchVariants>;
