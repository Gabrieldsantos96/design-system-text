...
import { selectContentVariants, selectTriggerVariants, type B3SelectTriggerVariants } from './select.variants';
...
@Component({
  selector: 'b3-select, [b3-select]',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [OverlayModule, B3IconComponent],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => B3SelectComponent),
      multi: true,
    },
  ],
  host: {
  ...
  template: `
    ...
    >
      ...
      <b3-icon zType="chevron-down" zSize="lg" class="opacity-50" />
    </button>
...
export class B3SelectComponent implements ControlValueAccessor, OnInit, AfterContentInit, OnDestroy {
  ...
  readonly selectItems = contentChildren(B3SelectItemComponent);

  private overlayRef?: OverlayRef;
  private portal?: TemplatePortal;

  readonly zSize = input<B3SelectTriggerVariants['zSize']>('default');
  ...
  private getSelectItems(): HTMLElement[] {
    ...
    return Array.from(dropdownElement.querySelectorAll<HTMLElement>('b3-select-item, [b3-select-item]')).filter(item => item.dataset['disabled'] === undefined);
  }
...
export const selectContentVariants = cva(
  'b3-[9999] max-h-96 min-w-[8rem] overflow-x-hidden overflow-y-auto rounded-md border bg-popover text-popover-foreground shadow-lg animate-in fade-in-0 zoom-in-95',
);
...
export type B3SelectTriggerVariants = VariantProps<typeof selectTriggerVariants>;
export type B3SelectContentVariants = VariantProps<typeof selectContentVariants>;
export type B3SelectItemVariants = VariantProps<typeof selectItemVariants>;
