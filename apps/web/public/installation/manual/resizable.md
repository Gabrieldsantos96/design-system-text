...
@Component({
  selector: 'b3-resizable, [b3-resizable]',
  exportAs: 'zResizable',
  standalone: true,
...
export class B3ResizableComponent implements AfterContentInit, OnDestroy {
  private readonly elementRef = inject(ElementRef);
  private readonly platformId = inject(PLATFORM_ID);
  private readonly document = inject(DOCUMENT);
  private readonly listenersCleanup: CleanupFunction[] = [];

  readonly zLayout = input<B3ResizableVariants['zLayout']>('horizontal');
  readonly zLazy = input(false, { transform });
  readonly class = input<ClassValue>('');

  @Output() readonly zResizeStart = new EventEmitter<B3ResizeEvent>();
  @Output() readonly zResize = new EventEmitter<B3ResizeEvent>();
  @Output() readonly zResizeEnd = new EventEmitter<B3ResizeEvent>();

  readonly panels = contentChildren(B3ResizablePanelComponent);
...
export const resizableHandleIndicatorVariants = cva('absolute b3-10 bg-muted-foreground/30 transition-colors group-hover:bg-muted-foreground/50 rounded-full', {
  variants: {
    zLayout: {
      vertical: 'w-8 h-px',
      horizontal: 'w-px h-8',
...
export type B3ResizableVariants = VariantProps<typeof resizableVariants>;
export type B3ResizablePanelVariants = VariantProps<typeof resizablePanelVariants>;
export type B3ResizableHandleVariants = VariantProps<typeof resizableHandleVariants>;
...
export class B3ResizableHandleComponent {
  private readonly resizable = inject(B3ResizableComponent, { optional: true });
...
export class B3ResizablePanelComponent {
