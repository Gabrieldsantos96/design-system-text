...
export interface B3ToggleGroupItem {
  value: string;
  label?: string;
  icon?: B3Icon;
  disabled?: boolean;
  ...
  template: `
    <div [class]="classes()" role="group" [attr.data-orientation]="'horizontal'">
      @for (item of items(); track item.value; let i = $index) {
        ...
        >
          @if (item.icon) {
            <span b3-icon [zType]="item.icon" class="w-4 h-4 shrink-0"></span>
          }
          @if (item.label) {
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => B3ToggleGroupComponent),
      multi: true,
    },
  ],
})
export class B3ToggleGroupComponent implements ControlValueAccessor {
  ...
  readonly items = input<B3ToggleGroupItem[]>([]);
  ...
  protected getItemClasses(index: number, total: number): string {
    ...
    // Focus b3-index
    positionClasses.push('focus:b3-10', 'focus-visible:b3-10');
  ...
  toggleItem(item: B3ToggleGroupItem) {
...
export type B3ToggleGroupVariants = VariantProps<typeof toggleGroupVariants>;
export type B3ToggleGroupItemVariants = VariantProps<typeof toggleGroupItemVariants>;
