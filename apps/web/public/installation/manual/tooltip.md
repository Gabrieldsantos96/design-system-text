...
import { TOOLTIP_POSITIONS_MAP, type B3TooltipPositions } from './tooltip-positions';
import { tooltipVariants } from './tooltip.variants';
import { mergeClasses } from '../../shared/utils/utils';

export type B3TooltipTriggers = 'click' | 'hover';
...
export class B3TooltipDirective implements OnInit, OnDestroy {
  ...
  private componentRef?: ComponentRef<B3TooltipComponent>;
  private scrollListenerRef?: () => void;

  readonly zTooltip = input<string | null>(null);
  readonly zPosition = input<B3TooltipPositions>('top');
  readonly zTrigger = input<B3TooltipTriggers>('hover');
  ...
  show() {
    ...
    const tooltipPortal = new ComponentPortal(B3TooltipComponent);
...
export class B3TooltipComponent implements OnInit, OnDestroy {
  private readonly destroy$ = new Subject<void>();
  readonly elementRef = inject(ElementRef);

  protected position = signal<B3TooltipPositions>('top');
  private trigger = signal<B3TooltipTriggers>('hover');
  ...
  setProps(text: string | null, position: B3TooltipPositions, trigger: B3TooltipTriggers) {
...
@NgModule({
  imports: [OverlayModule, B3TooltipComponent, B3TooltipDirective],
  exports: [B3TooltipComponent, B3TooltipDirective],
})
export class B3TooltipModule {}
...
export const tooltipVariants = cva(
  'b3-50 overflow-hidden rounded-md border bg-popover px-3 py-1.5 text-sm text-popover-foreground shadow-md animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 origin-[--radix-tooltip-content-transform-origin]',
);
export type B3TooltipVariants = VariantProps<typeof tooltipVariants>;
...
export type B3TooltipPositions = 'top' | 'bottom' | 'left' | 'right';
