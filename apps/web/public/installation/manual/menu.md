...
import { B3MenuManagerService } from './menu-manager.service';
import { MENU_POSITIONS_MAP, type B3MenuPlacement } from './menu-positions';

export type B3MenuTrigger = 'click' | 'hover';
...
export class B3MenuDirective implements OnInit, OnDestroy {
  private static readonly MENU_CONTENT_SELECTOR = '.cdk-overlay-pane [b3-menu-content]';

  protected readonly cdkTrigger = inject(CdkMenuTrigger, { host: true });
  private readonly elementRef = inject(ElementRef);
  private readonly menuManager = inject(B3MenuManagerService);
  ...
  readonly zTrigger = input<B3MenuTrigger>('click');
  readonly zHoverDelay = input<number>(100);
  readonly zPlacement = input<B3MenuPlacement>('bottomLeft');
  ...
  private getPositionsByPlacement(placement: B3MenuPlacement): ConnectedPosition[] {
  ...
  private setupMenuContentListeners(): void {
    const menuContent = document.querySelector(B3MenuDirective.MENU_CONTENT_SELECTOR);
  ...
  private shouldKeepMenuOpen(relatedTarget: Element | null): boolean {
    ...
    const isMovingToMenu = relatedTarget.closest(B3MenuDirective.MENU_CONTENT_SELECTOR);
    const isMovingToOtherTrigger = relatedTarget.matches('[b3-menu]') && !this.elementRef.nativeElement.contains(relatedTarget);
...
export type B3MenuContentVariants = VariantProps<typeof menuContentVariants>;
export type B3MenuItemVariants = VariantProps<typeof menuItemVariants>;
...
export class B3MenuContentDirective {
...
export class B3MenuItemDirective {
  private readonly cdkMenuItem = inject(CdkMenuItem, { host: true });

  readonly zDisabled = input<boolean, BooleanInput>(false, { transform: booleanAttribute });
  readonly zInset = input<B3MenuItemVariants['inset']>(false);
  readonly class = input<ClassValue>('');
