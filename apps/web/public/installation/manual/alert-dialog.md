...
export const alertDialogVariants = cva('fixed b3-50 w-full max-w-[calc(100%-2rem)] border bg-background shadow-lg rounded-lg sm:max-w-lg');

export type B3AlertDialogVariants = VariantProps<typeof alertDialogVariants>;
...
import type { OnClickCallback, B3AlertDialogComponent, B3AlertDialogOptions } from './alert-dialog.component';

export class B3AlertDialogRef<T = unknown, R = unknown> {
  componentInstance?: T;
  ...
  constructor(
    ...
    private readonly config: B3AlertDialogOptions<T>,
    private readonly containerInstance: B3AlertDialogComponent<T>,
  ) {
...
<div class="flex flex-col gap-4 p-6" cdkTrapFocus [cdkTrapFocusAutoCapture]="true">
  @if (config.zTitle || config.zDescription) {
    <header class="flex flex-col gap-2 text-center sm:text-left">
      @if (config.zTitle) {
        <h2 data-testid="b3-alert-title" [id]="titleId()" class="text-lg font-semibold">{{ config.zTitle }}</h2>
      }

      @if (config.zDescription) {
        <p data-testid="b3-alert-description" [id]="descriptionId()" class="text-sm text-muted-foreground">{{ config.zDescription }}</p>
      }
    </header>
  }
  ...
  <main>
    ...
    @if (isStringContent) {
      <div data-testid="b3-alert-content" [innerHTML]="config.zContent"></div>
    }
  </main>

  <footer class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
    @if (config.zCancelText !== null) {
      <button data-testid="b3-alert-cancel-button" b3-button zType="outline" (click)="onCancelClick()">
        {{ config.zCancelText || 'Cancel' }}
      </button>
    }

    @if (config.zOkText !== null) {
      <button data-testid="b3-alert-ok-button" b3-button [zType]="config.zOkDestructive ? 'destructive' : 'default'" [disabled]="config.zOkDisabled" (click)="onOkClick()">
