...
import { segmentedItemVariants, segmentedVariants, type B3SegmentedVariants } from './segmented.variants';
...
@Component({
  selector: 'b3-segmented-item',
  standalone: true,
  template: `<ng-content></ng-content>`,
  encapsulation: ViewEncapsulation.None,
})
export class B3SegmentedItemComponent {
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => B3SegmentedComponent),
      multi: true,
    },
  ],
})
export class B3SegmentedComponent implements ControlValueAccessor, OnInit {
  private readonly itemComponents = contentChildren(B3SegmentedItemComponent);

  readonly class = input<ClassValue>('');
  readonly zSize = input<B3SegmentedVariants['zSize']>('default');
  ...
  protected readonly items = signal<readonly B3SegmentedItemComponent[]>([]);
...
export type B3SegmentedVariants = VariantProps<typeof segmentedVariants>;
export type B3SegmentedItemVariants = VariantProps<typeof segmentedItemVariants>;
