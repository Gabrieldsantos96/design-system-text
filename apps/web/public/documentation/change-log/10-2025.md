...
If you're using B3UI icons in your project, you need to update from CSS classes to lucide-angular components:
...
Or using B3UI's icon component:

```angular-ts
import { B3IconComponent } from '@b3/components/icon';
import { Home } from 'lucide-angular';

@Component({
  imports: [B3IconComponent],
  template: `<div b3-icon [zType]="HomeIcon"></div>`
})
export class MyComponent {
  readonly HomeIcon = Home;
}
...
```angular-ts
import { B3SheetService } from '@b3/components/sheet';
...
```angular-ts
import { B3EmptyComponent } from '@shared/components/empty';
