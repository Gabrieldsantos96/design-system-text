...
@Component({
  selector: 'b3-demo-icon-searchable',
  standalone: true,
  ...
  template: `
    <div class="flex w-full flex-col gap-4">
      ...
      @if (filteredIcons().length === 0) {
        <b3-empty zDescription="No icons found for the given search." />
      }
    </div>
  `,
})
export class B3DemoIconSearchableComponent {
  ...
  async copyIconCode(iconName: string): Promise<void> {
    const code = `<b3-icon zType="${iconName}" />`;

    try {
      await navigator.clipboard.writeText(code);

      toast.success('Icon copied!', {
        description: `<b3-icon zType="${iconName}" />`,
        duration: 2000,
      });
    } catch (err) {
