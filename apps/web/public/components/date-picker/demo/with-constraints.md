  ...
  template: `
    <div class="flex flex-col gap-4">
      <div class="space-y-2">
        ...
        <b3-date-picker [value]="selectedDateRange()" placeholder="Pick a date within range" [minDate]="minDate()" [maxDate]="maxDate()" (dateChange)="onDateChangeRange($event)" />
      </div>

      <div class="space-y-2">
        <h4 class="text-sm font-medium">Disabled</h4>
        <b3-date-picker placeholder="Disabled date picker" [disabled]="true" />
      </div>

      <div class="space-y-2">
        <h4 class="text-sm font-medium">With Initial Value</h4>
        <b3-date-picker [value]="selectedDateInitial()" placeholder="Pick a date" (dateChange)="onDateChangeInitial($event)" />
      </div>
    </div>
  `,
})
export class B3DemoDatePickerWithConstraintsComponent {
