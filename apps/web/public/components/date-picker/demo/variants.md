  ...
  template: `
    <div class="flex flex-col gap-4">
      <div class="space-y-2">
        ...
        <b3-date-picker zType="default" [value]="selectedDateDefault()" placeholder="Pick a date" (dateChange)="onDateChangeDefault($event)" />
      </div>

      <div class="space-y-2">
        <h4 class="text-sm font-medium">Outline</h4>
        <b3-date-picker zType="outline" [value]="selectedDateOutline()" placeholder="Pick a date" (dateChange)="onDateChangeOutline($event)" />
      </div>

      <div class="space-y-2">
        <h4 class="text-sm font-medium">Ghost</h4>
        <b3-date-picker zType="ghost" [value]="selectedDateGhost()" placeholder="Pick a date" (dateChange)="onDateChangeGhost($event)" />
      </div>
    </div>
  `,
})
export class B3DemoDatePickerVariantsComponent {
